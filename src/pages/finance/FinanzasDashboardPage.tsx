                    ...
                    <Paper sx={{ 
                        ...
                        <Box sx={{ display: 'flex', gap: 2, height: 200, alignItems: 'flex-end', justifyContent: 'space-around' }}>
                            {[40, 60, 45, 80, 55, 70, 90].map((v, i) => (
                                <Box key={i} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                                    ...
                                    <Typography sx={{ fontSize: '10px', color: 'var(--text-subtle, #6b7280)' }}>Día {i + 1}</Typography>
                                </Box>
                            ))}
                        </Box>
                        <Box sx={{ mt: 4, display: 'flex', justifyContent: 'center', gap: 4 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                ...
                                <Typography sx={{ fontSize: '12px', color: 'var(--text-muted, #9ca3af)' }}>Ingresos Proyectados</Typography>
                            </Box>
                        </Box>
                    </Paper>
                </Grid>
                        ...
                        {projections.map((p) => (
                            <Box key={p.label} sx={{ mb: 4 }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                                    <Typography variant="body2" sx={{ color: 'var(--text-muted, #9ca3af)' }}>{p.label}</Typography>
                                    <Typography variant="body2" sx={{ fontWeight: 700, color: p.color }}>{p.value}</Typography>
                                </Box>
                                <LinearProgress 
                                    variant="determinate" 
                                    value={70} 
                                    sx={{ height: 6, borderRadius: 3, bgcolor: 'var(--bg-action-btn, rgba(255,255,255,0.05))', '& .MuiLinearProgress-bar': { bgcolor: p.color } }}
                                />
                            </Box>
                        ))}
                        <Typography variant="caption" sx={{ color: 'var(--text-subtle, #6b7280)', fontStyle: 'italic' }}>
