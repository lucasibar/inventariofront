        ...
        <div style={{ padding: isMobile ? '12px' : '24px', maxWidth: '1100px', margin: '0 auto' }}>
                ...
                {executiveKpis.map((kpi) => (
                    <Card key={kpi.title} style={{ padding: '16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                            ...
                            <span style={{ fontSize: '11px', color: 'var(--text-subtle, #6b7280)', fontWeight: 600 }}>KPI</span>
                        </div>
                        <div style={{ color: kpi.color, fontSize: '22px', fontWeight: 800, marginBottom: '4px' }}>
                            {kpi.value}
                        </div>
                        <div style={{ color: 'var(--text-primary, #f3f4f6)', fontSize: '12px', fontWeight: 700 }}>
                            {kpi.title}
                        </div>
                        <div style={{ color: 'var(--text-subtle, #6b7280)', fontSize: '11px', marginTop: '2px' }}>
                            {kpi.subtitle}
                        </div>
                    </Card>
                ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 2fr', gap: '16px', marginBottom: '24px' }}>
                <Card style={{ padding: '20px' }}>
                    <h3 style={{ color: 'var(--text-primary, #f3f4f6)', fontSize: '15px', fontWeight: 700, margin: '0 0 14px' }}>
                    ...
                    {Object.keys(issueBreakdown).length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {Object.entries(issueBreakdown).map(([issue, count]) => (
                                <div key={issue} style={{
                                    ...
                                    border: '1px solid var(--border-color, #2a2d3e)', display: 'flex', justifyContent: 'space-between', alignItems: 'center'
                                }}>
                                    <span style={{ color: 'var(--text-secondary, #d1d5db)', fontSize: '12px', fontWeight: 600 }}>{issue}</span>
                        ...
                        <div style={{ color: 'var(--text-subtle, #6b7280)', fontSize: '13px', textAlign: 'center', padding: '20px 0' }}>
                ...
                <Card style={{ padding: '0', overflow: 'hidden' }}>
                    <div style={{
                        padding: '16px 20px', borderBottom: '1px solid var(--border-color, #2a2d3e)',
                        ...
                        <div style={{ display: 'flex', gap: '6px' }}>
                            {(['ALL', 'COMPLETADO', 'EN_PROGRESO'] as const).map(st => (
                                <button
                                    ...
                                    style={{
                                        background: statusFilter === st ? 'rgba(99,102,241,0.2)' : 'var(--bg-primary, #0f1117)',
                                        border: `1px solid ${statusFilter === st ? '#6366f1' : 'var(--border-strong, #374151)'}`,
                                        color: statusFilter === st ? '#a5b4fc' : 'var(--text-muted, #9ca3af)',
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
                            <thead>
                                <tr style={{ background: 'var(--bg-primary, #0f1117)', color: 'var(--text-muted, #9ca3af)', borderBottom: '1px solid var(--border-color, #2a2d3e)' }}>
                                ...
                                {filteredHistory.length > 0 ? (
                                    filteredHistory.map((item) => (
                                        <tr key={item.id} style={{ borderBottom: '1px solid #1f2233' }}>
                                            ...
                                            <td style={{ padding: '12px 16px', color: 'var(--text-muted, #9ca3af)' }}>
                                                {new Date(item.startedAt).toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                            </td>
                                            <td style={{ padding: '12px 16px', color: 'var(--text-secondary, #d1d5db)', fontWeight: 600 }}>
                                    ...
                                    <tr>
                                        <td colSpan={7} style={{ padding: '30px', textAlign: 'center', color: 'var(--text-subtle, #6b7280)' }}>
