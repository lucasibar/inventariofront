                        ...
                        <Stack spacing={1.25}>
                            ...
                            {request.lines.map((line) => {
                                ...
                                return <Paper key={line.id} variant="outlined" sx={{ p: { xs: 1.5, sm: 2 }, borderRadius: 3 }}>
                                    <Stack direction="row" justifyContent="space-between" spacing={1}><Box><Typography fontWeight={900}>{line.item.codigoInterno} · {line.item.descripcion}</Typography><Typography variant="caption" color="text.secondary">Pedido {Number(line.requestedKg).toFixed(3)} kg · Entregado {Number(line.deliveredKg).toFixed(3)} kg · Consumido {Number(line.consumedKg).toFixed(3)} kg</Typography></Box><Chip size="small" label={`${Number(line.reservedKg).toFixed(2)} kg reservados`} color={Number(line.reservedKg) >= Number(line.requestedKg) ? 'success' : 'default'} /></Stack>
