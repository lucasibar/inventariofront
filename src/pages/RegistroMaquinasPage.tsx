...
import { PageHeader, Spinner, Select } from './common/ui';
...
export default function RegistroMaquinasPage() {
    ...
    useEffect(() => {
        if (machineTypes.length > 0 && !selectedTypeId) {
            const tej = machineTypes.find(t => t.name.toLowerCase().includes('tejedur'));
    ...
    useEffect(() => {
        if (user) {
            setValue('generatedBy', user.name || user.username || '');
        }
    }, [user, setValue]);
    ...
    const typeOptions = useMemo(() => machineTypes.map(t => ({ value: t.id, label: t.name })), [machineTypes]);
    const machineOptions = useMemo(() => machines.map(m => ({ value: m.id, label: `Máquina ${m.number} - ${m.codigoInterno || ''}` })), [machines]);
                        ...
                        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3, mb: 4 }}>
                            ...
                            <Box sx={{ gridColumn: { md: '1 / span 2' } }}>
                                <Autocomplete
                                    ...
                                    value={machineOptions.find(o => o.value === selectedMachineId) || null}
