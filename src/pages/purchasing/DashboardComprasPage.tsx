...
export default function DashboardComprasPage() {
    ...
    const alerts = useMemo(() => {
        if (!depotId) return rawAlerts;
        return rawAlerts.filter((a: any) => a.depositoId === depotId);
    }, [rawAlerts, depotId]);
        ...
        <Box sx={{ bgcolor: colors.bg, minHeight: '100vh', color: colors.text, pb: 10, maxWidth: '1400px', margin: '0 auto' }}>
            ...
            <Box sx={{ p: 2, pb: 1, display: 'flex', gap: 1.5, flexWrap: { xs: 'wrap', sm: 'nowrap' } }}>
                ...
                <TextField 
                    ...
                    fullWidth 
                    InputProps={{ 
                        startAdornment: <SearchIcon sx={{ color: colors.textDim, mr: 1 }} />, 
                        endAdornment: browserSupportsSpeechRecognition && (
                            <IconButton size="small" onClick={toggleListening} sx={{ color: listening ? colors.danger : colors.textDim }}>
            ...
            <NewOrderDrawer open={newOrderOpen} onClose={() => setNewOrderOpen(false)} suppliers={suppliers} items={items} depots={depots} defaultDepotId={depotId} />
        </Box>
    );
}
...
const NewOrderDrawer = ({ open, onClose, suppliers, items, depots, defaultDepotId }: any) => {
    ...
    const handleSave = async () => {
        if (!form.supplierId || !form.depositoId || lines.some(l => !l.itemId || !l.qtyPedido)) { 
            alert('Completá todos los campos (incluyendo el depósito y proveedor).'); 
            return; 
        }
        try { 
            await createOrder({ 
                ...form, 
                lines: lines.map(l => ({ itemId: l.itemId, qtyPedido: Number(l.qtyPedido) })) 
            }).unwrap(); 
            onClose(); 
        } catch (e: any) { 
            alert(e?.data?.message || 'Error al crear OC'); 
        }
    };
    return (
