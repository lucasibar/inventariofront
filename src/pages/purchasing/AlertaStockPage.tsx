        ...
        <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            {isLoading ? <Spinner /> : alerts.length > 0 ? (
                <Card style={{ padding: '0' }}>
                    <Table 
                        ...
                        rows={alerts.map((a: any) => [
                            <strong key="desc" style={{ color: 'var(--text-primary, #f3f4f6)' }}>{a.descripcion}</strong>,
                            ...
                            <span key="min" style={{ color: 'var(--text-subtle, #6b7280)' }}>{Number(a.stockMinimo).toFixed(1)} kg</span>,
                ...
                <div style={{ textAlign: 'center', padding: '60px', background: 'rgba(52, 211, 153, 0.05)', borderRadius: '12px', border: '1px dashed rgba(52, 211, 153, 0.3)' }}>
                    ...
                    <p style={{ color: 'var(--text-subtle, #6b7280)', fontSize: '14px', marginTop: '8px' }}>No hay materiales por debajo del stock mínimo configurado.</p>
                </div>
            )}
        </div>
    );
