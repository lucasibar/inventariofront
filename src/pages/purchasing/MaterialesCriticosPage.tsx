...
} from '../../features/purchasing/combos/api/combos.api';
...
export default function MaterialesCriticosPage() {
    ...
    const { data: allStock = [] } = useGetStockQuery({ depotId: depotId || undefined });
    ...
    const [newCombo, setNewCombo] = useState<{ title: string; supplierId: string; itemIds: string[]; depositoId: string }>({
        title: '',
        supplierId: '',
        itemIds: [],
        depositoId: depotId || ''
    ...
    useEffect(() => {
        if (depotId) {
            setNewCombo(prev => ({ ...prev, depositoId: depotId }));
        }
    }, [depotId]);
    ...
    const handleCreate = async () => {
        ...
        await createCombo({
            ...
            depositoId: newCombo.depositoId
        });
        setShowCreateModal(false);
        setNewCombo({ title: '', supplierId: '', itemIds: [], depositoId: depotId || '' });
    };
    ...
    return (
        <div className="dashboard-container" style={{ padding: isMobile ? '12px' : '24px', maxWidth: '1400px', margin: '0 auto' }}>
                            ...
                            <div className="combos-grid">
                                {filteredCombos.map((combo: any) => (
                                    <ComboCard 
                                        ...
                                        combo={combo} 
                                        onClick={() => setShowBreakdownId(combo.id)}
                                        onUpdateTitle={(title: string) => updateCombo({ id: combo.id, data: { title } })}
            ...
            {editComboId && <EditComboModal combo={combos.find((c: any) => c.id === editComboId)} items={items} onClose={() => setEditComboId(null)} onSave={async (ids: string[]) => { await updateCombo({ id: editComboId, data: { itemIds: ids } }); setEditComboId(null); }} />}
        </div>
    );
}
