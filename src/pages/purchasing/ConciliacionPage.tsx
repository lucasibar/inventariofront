    ...
    return (
        <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '24px' }}>
                <div>
                    <h3 style={{ color: 'var(--text-primary, #f3f4f6)', fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>📦 Entradas sin vincular</h3>
                    {loadingMovs ? <Spinner /> : unlinkedMovements.length === 0 ? (
                        <Card style={{ textAlign: 'center', padding: '40px' }}>
                            <p style={{ color: 'var(--text-subtle, #6b7280)' }}>No hay remitos pendientes de conciliación.</p>
                        </Card>
                    ) : (
                            ...
                            {unlinkedMovements.map((m: any) => (
                                ...
                                >
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <div>
                                            <div style={{ fontWeight: 700, color: 'var(--text-primary, #f3f4f6)', fontSize: '15px' }}>{m.item?.descripcion}</div>
                                            <div style={{ fontSize: '12px', color: 'var(--text-subtle, #6b7280)', marginTop: '4px' }}>
                                        ...
                                        <div style={{ textAlign: 'right' }}>
                                            ...
                                            <div style={{ fontSize: '11px', color: 'var(--text-subtle, #6b7280)' }}>Recibido: {new Date(m.fecha).toLocaleDateString()}</div>
                                        </div>
                                    </div>
                                </Card>
                            ))}
