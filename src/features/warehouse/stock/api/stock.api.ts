...
export const stockApi = api.injectEndpoints({
    endpoints: (builder) => ({
        ...
        createCombo: builder.mutation<any, { title: string; supplierId?: string | null; itemIds: string[]; depositoId?: string | null }>({
        ...
        updateCombo: builder.mutation<any, { id: string; title?: string; itemIds?: string[]; depositoId?: string | null }>({
        ...
        reverseMovement: builder.mutation<void, string>({
            query: (id: string) => ({ url: `movimientos/${id}/cancel`, method: 'POST' }),
            invalidatesTags: ['Stock'],
        }),
